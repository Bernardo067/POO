...
class Produto:
	...
	def preco(self, novo_preco):
		if isinstance(novo_preco, int, float) == True:
			if novo_preco > 0:
...
class ProdutoFisico(Produto):
	...
	def peso(self, novo_peso):
		if isinstance(novo_peso, int) == True:
			if novo_peso > 0 :
	...
	def calcular_preco_com_frete(self):
		divz = self.__peso / 1000
		frete = divz * 5
		preco_final = self.__preco + frete
		return preco_final
		"""
...
class ProdutoEletronico(ProdutoFisico):
	...
	def tensao(self, nova_tensao):
		if isinstance(nova_tensao, int,) == True:
			if nova_tensao == 127 or nova_tensao == 220:
	...
	def calcular_preco_com_frete(self):
		divz = self.__peso // 1000
		subtotal = self.__preco + (divz * 5)
		preco_final = subtotal + (subtotal/100)
		return preco_final
		"""
...
class Ebook(Produto):
	...
	def nome_exibicao(self):
		nome = f"{self.__nome} ({self.__autor})"
		return nome
